...
.hero-nav { display: flex; align-items: center; margin-bottom: 36px; }
.hero-nav-link {
...
.hl-line { font-size: clamp(52px, 6vw, 88px); color: #ffffff; }
...
.hero-moto-wrap { position: relative; width: 100%; max-width: 580px; }
.hero-moto-img {
    width: 100%;
    height: auto;
    display: block;
...
.hero-stats-bar { display: flex; align-items: center; justify-content: center; }
.hsb-item { display: flex; flex-direction: column; align-items: center; padding: 0 36px; }
.hsb-num {
    font-family: Orbitron, sans-serif;
    font-weight: 800;
    font-size: 22px;
...
.hsb-sep { width: 1px; height: 32px; background: rgba(255,255,255,0.08); }
...
with open('resources/css/dashboard.css', 'r', encoding='utf-8') as f:
    lines = f.readlines()

# before: lines 0-131 (everything BEFORE the HERO section header)
before = lines[:132]
# after: lines starting from BRAND section (line 191 in original = index 190)
# Find the BRAND section comment line
brand_idx = None
for i in range(150, len(lines)):
    if 'BRAND' in lines[i] and '   BRAND' in lines[i].rstrip():
        brand_idx = i - 1
        break

if brand_idx is None:
    print("ERROR: Could not find BRAND section")
    exit(1)

after = lines[brand_idx:]
print(f"BRAND section at line {brand_idx+1}")

result = ''.join(before) + '\n' + new_hero.strip() + '\n' + ''.join(after)

with open('resources/css/dashboard.css', 'w', encoding='utf-8') as f:
    f.write(result)

# Copy to build
import shutil
shutil.copy('resources/css/dashboard.css', 'public/build/css/dashboard.css')

print(f"Done. Source lines: {len(result.splitlines())}, Build lines: {len(open('public/build/css/dashboard.css').read().splitlines())}")
