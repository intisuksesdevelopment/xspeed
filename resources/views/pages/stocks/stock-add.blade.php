                    ...
                    <div class="card-body add-product pb-0">
                        ...
                        <div class="col-lg-12">
                            <div class="btn-addproduct mb-4">
                                ...
                                <button type="submit" class="btn btn-submit" id="submit-add-button">Save Stock
                                    Opname</button>
                            </div>
                        </div>
                    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            async function fetchDropdownsData() {
                try {
                    const [warehousesRes, categoriesRes, subcategoriesRes, brandsRes, racksRes] = await Promise
                        .all([
                    ...
                    // Handle Laravel JsonResponse format (original property) or direct array or {success, data} format
                    function extractData(response) {
                        // Debug: log the response to see its structure
                        console.log('Response received:', JSON.stringify(response).substring(0, 500));

                        // If response has 'original' property (Laravel JsonResponse), use it
                        if (response && response.original) {
                            const original = response.original;
                            console.log('Original:', JSON.stringify(original).substring(0, 500));
                            // If original has 'data' property (our custom format)
                            if (Array.isArray(original.data)) return original.data;
                            // If original is an array
                            if (Array.isArray(original)) return original;
                            // If original has 'results' (Select2 format), return empty array (not for dropdowns)
                            if (Array.isArray(original.results)) {
                                console.log('Select2 format detected, returning empty for dropdown');
                                return [];
                            }
                        }
                        // If response has 'data' property (our custom format)
                    ...
                    warehouses = extractData(warehousesData);
                    console.log('Warehouses extracted:', warehouses.length, 'items');
                    categories = extractData(categoriesData);
                    subcategories = extractData(subcategoriesData);
                    brands = extractData(brandsData);
                    racks = extractData(racksData);
